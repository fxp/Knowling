...
**只做「单个知识点的学习 + Quiz」交互组件。** 输入一个细小知识点 → 输出一个自包含、可交互的学习组件（叙述 + explorable 演示 + 测验）。

**明确不做**：大主题自动拆解（多知识点）、知识图谱/导航外壳、整门课/整本书生成。即原计划 **P3 移出范围**。`concept_graph`/`timeline` 仅作单组件内的学习辅助，不作跨知识点导航。（呼应设计文档 D1：知识点→单组件。）
...
| **P2.5 Quiz 强化** | 多题型 + 多题计分测验 | ✅ 完成 |
| **~~P3 拆解+导航~~** | ~~大主题拆解 + 知识图谱外壳~~ | ✂︎ **移出范围**（见上）|
| **P4 接口化** | MCP Tool + SKILL.md + Server API（单组件生成）| 🟡 部分（SKILL.md / CLI 已就绪）|
...
> 测验是「学习 + Quiz」组件的另一半，单 KP 范围内的高价值打磨。

- [x] 四种题型：`single` / `multi`（多选，精确集合匹配）/ `boolean`（判断，归一化为二选一）/ `fill`（填空，answer + accept[] 大小写不敏感匹配）
- [x] 多题测验集：进度、逐题提交判定、最终计分、重做
- [x] 单题向后兼容（`{question, options, answer, explain}` 仍可用）
- [x] 选中态 / 提交 / 下一题 / 成绩 UI + `qa_assertions` 按题型与题量扩展
- [x] 真实浏览器端到端验证（选择→提交→判定→下一题→计分）

## ~~P3 · 拆解 + 导航~~（移出范围）

不做。属于「多知识点 / 整课」方向，与产品 focus（单知识点学习+Quiz）相悖。若未来确有需要，由宿主系统在 Knowling 之上自行编排，不进本模块。
