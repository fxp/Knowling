...
def test_refine_keeps_kp_id_and_bumps_version():
    ...
    new_spec, call, summary = refine.refine(base.spec, kp, "随便", get_provider("mock", quiet=True))
...
def test_refine_adds_relevant_block(instruction, expect_type):
    cfg, kp = _cfg(), _kp()
    base = generate_knowling(kp, cfg)
    new, summary = refine_knowling(base.spec, kp, instruction, cfg)
...
def test_refine_produces_a_new_distinct_card():
    cfg, kp = _cfg(), _kp()
    base = generate_knowling(kp, cfg)
    new, _ = refine_knowling(base.spec, kp, "太难了", cfg)
