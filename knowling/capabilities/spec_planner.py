...
1. 先定 pedagogy: hook(抓手)、central_phenomenon(要引导注意的中心现象)、
   ...
   prerequisites(前置知识点, 2-4 个, 数组)。**prerequisites 很重要**: 列出学这张卡
   之前**必须先掌握**的、本卡直接依赖的更基础知识点; 每项是对象 {{"name": 前置知识点名称,
   "why": 一句话说明本卡哪里会用到它}}。它会被渲染成**整张卡的第一页**(学习之前先看)。
   只列真正的直接前置(够用即可), 由浅入深排序; 若确实零前置, 给空数组。
...
pedagogy.prerequisites 形如:
  [{{"name": "正弦函数 y=sin x 的图象与周期", "why": "本卡在它基础上加入 A、ω、φ 三个参数"}}, ...]
