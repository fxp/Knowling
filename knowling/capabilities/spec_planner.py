...
2. 选 4-6 个 block 并排序, **尽量用多种模态让一张卡有层次**, 不要只有
   text+param_sim+quiz 三件套。尽量覆盖下列模态(按需取舍, 至少 3 类):
   - 启发/直觉: hook + callout(生活类比、为什么重要、反直觉之处)
   - 理论/概念: text(讲清定义、原理、关键公式)
   - 可视化+交互(重心): param_sim / interactive_demo(拖动看曲线/数值变化)、
     step_through(逐步推演证明或过程)
   - 听觉(适用时): audio 块——涉及频率/振幅/音高/响度/声波/振动/周期 的知识点,
     务必加一个 audio 块, 让读者拖滑块边看波形边"听见"变化(频率→音高, 振幅→响度)
   - 实例: text 或 callout(一个具体例子完整走一遍, 代入真实数字)
   - 评测: quiz(检验是否真懂, 可多题/多题型)
   **尽量用可视化呈现**: 凡是函数/几何/随量变化的知识点, 一定要有 param_sim 的
   二维坐标系曲线; 优先让读者"动手改变某个量并观察(或听到)结果", 而非只读文字。
